using Microsoft.VisualStudio.TestTools.UnitTesting;
using System;
using Tyuiu.GoginMA.Sprint6.Task5.V13.Lib;

namespace Tyuiu.GoginMA.Sprint6.Task5.V13.Test
{
    [TestClass]
    public class DataServiceTest
    {
        [TestMethod]
        public void LoadFromDataFile()
        {
            DataService ds = new DataService();
            string path = @"C:\Users\Misha\source\repos\Tyuiu.GoginMA.Sprint6\Tyuiu.GoginMA.Sprint6.Task5.V13\bin\Debug\InPutFileTask5V13.txt";

            CollectionAssert.AreEqual(new double[] { -13, -19, -9.82, -9.71, 3.36, 0.48, 4.13, -0.11, -17.36, -12, -12.35 }, ds.LoadFromDataFile(path));
        }
    }
}
